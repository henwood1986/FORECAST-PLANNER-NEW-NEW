...
import { PlusCircle, Trash2, TrendingUp, Calendar, Download, Upload, BarChart3 } from 'lucide-react';
...
const CSMForecastPlanner = () => {
    ...
    scenarios.forEach(scenario => {
      ...
      filteredCustomers.forEach(customer => {
        ...
        if (!monthlyData[monthKey]) {
          monthlyData[monthKey] = { startingARR: 0, renewedARR: 0, upsell: 0, churnedARR: 0 };
        }
        if (!quarterlyData[quarterKey]) {
          quarterlyData[quarterKey] = { startingARR: 0, renewedARR: 0, upsell: 0, churnedARR: 0 };
        }
        ...
        const churnedAmount = customer.currentARR * (1 - renewalPercent);
        ...
        monthlyData[monthKey].churnedARR += churnedAmount;
        ...
        quarterlyData[quarterKey].churnedARR += churnedAmount;
      });
      ...
      const totalChurn = filteredCustomers.reduce((sum, c) => {
        const renewalPercent = c[scenario].renewalPercent / 100;
        return sum + (c.currentARR * (1 - renewalPercent));
      }, 0);
  ...
  const csmMetrics = useMemo(() => {
    ...
    csms.forEach(csm => {
      const csmCustomers = customers.filter(c => c.csmName === csm);
      ...
      csmCustomers.forEach(customer => {
        const renewalPercent = customer[scenario].renewalPercent / 100;
        totalARR += customer.currentARR;
        renewedARR += customer.currentARR * renewalPercent;
        upsell += customer[scenario].upsell;
        churnedARR += customer.currentARR * (1 - renewalPercent);
      });
      ...
      csmData[csm] = {
        ...
        upsell,
        churnedARR,
        grr: (renewedARR / totalARR) * 100,
        nrr: ((renewedARR + upsell) / totalARR) * 100
      };
    });

    return Object.keys(csmData).sort().map(csm => ({
      csm,
      ...csmData[csm]
    }));
        ...
        {calculateMetrics && (
          <>
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-slate-600">Chart Scenario:</label>
                <select
                ...
                >
                  ...
                  <option value="mostLikely">Most Likely</option>
              ...
              {Object.keys(scenarioLabels).map(scenario => (
                <div key={scenario} className={`rounded-lg shadow-lg p-6 border-2 ${scenarioColors[scenario]}`}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <div className="text-sm text-slate-600">Total Expansion</div>
                      <div className="text-2xl font-bold text-green-600">{formatCurrency(calculateMetrics[scenario].overall.upsell)}</div>
                    </div>
                    <div>
                      <div className="text-sm text-slate-600">Total Churn</div>
                      <div className="text-2xl font-bold text-red-600">{formatCurrency(calculateMetrics[scenario].overall.churnedARR)}</div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              {/* GRR & NRR by Month Chart */}
              <div className="bg-white rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2">
                  <TrendingUp className="w-5 h-5" />
                  GRR & NRR by Month ({scenarioLabels[selectedScenario]})
                </h3>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={calculateMetrics[selectedScenario].monthly}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="period" />
                    <YAxis />
                    <Tooltip formatter={(value) => formatPercent(value)} />
                    <Legend />
                    <Line type="monotone" dataKey="grr" stroke="#3b82f6" name="GRR" strokeWidth={2} />
                    <Line type="monotone" dataKey="nrr" stroke="#10b981" name="NRR" strokeWidth={2} />
                  </LineChart>
                </ResponsiveContainer>
              </div>

              {/* ARR Breakdown by Month Chart */}
              <div className="bg-white rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2">
                  <BarChart3 className="w-5 h-5" />
                  ARR Breakdown by Month ({scenarioLabels[selectedScenario]})
                </h3>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={calculateMetrics[selectedScenario].monthly}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="period" />
                    <YAxis />
                    <Tooltip formatter={(value) => formatCurrency(value)} />
                    <Legend />
                    <Bar dataKey="renewedARR" fill="#3b82f6" name="Renewed" />
                    <Bar dataKey="upsell" fill="#10b981" name="Expansion" />
                    <Bar dataKey="churnedARR" fill="#ef4444" name="Churn" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>

            {/* CSM Performance Chart */}
            {csmMetrics && csmMetrics.length > 0 && (
              <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
                <h3 className="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2">
                  <BarChart3 className="w-5 h-5" />
                  Performance by CSM ({scenarioLabels[selectedScenario]})
                </h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={csmMetrics}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="csm" />
                      <YAxis />
                      <Tooltip formatter={(value) => formatPercent(value)} />
                      <Legend />
                      <Bar dataKey="grr" fill="#3b82f6" name="GRR %" />
                      <Bar dataKey="nrr" fill="#10b981" name="NRR %" />
                    </BarChart>
                  </ResponsiveContainer>
                    ...
                    {calculateMetrics.bestCase.quarterly.map((quarter, idx) => (
                      <React.Fragment key={quarter.period}>
                        <tr className="border-b bg-green-50">
                          ...
                          <td className="px-4 py-3 text-right text-green-600">{formatCurrency(quarter.upsell)}</td>
                          <td className="px-4 py-3 text-right text-red-600">{formatCurrency(quarter.churnedARR)}</td>
                        ...
                        <tr className="border-b bg-blue-50">
                          ...
                          <td className="px-4 py-3 text-right text-green-600">{formatCurrency(calculateMetrics.mostLikely.quarterly[idx].upsell)}</td>
                          <td className="px-4 py-3 text-right text-red-600">{formatCurrency(calculateMetrics.mostLikely.quarterly[idx].churnedARR)}</td>
                        ...
                        <tr className="border-b bg-orange-50">
                          ...
                          <td className="px-4 py-3 text-right text-green-600">{formatCurrency(calculateMetrics.worstCase.quarterly[idx].upsell)}</td>
                          <td className="px-4 py-3 text-right text-red-600">{formatCurrency(calculateMetrics.worstCase.quarterly[idx].churnedARR)}</td>
                    ...
                    {calculateMetrics.bestCase.monthly.map((month, idx) => (
                      <React.Fragment key={month.period}>
                        <tr className="border-b bg-green-50">
                          ...
                          <td className="px-4 py-3 text-right text-green-600">{formatCurrency(month.upsell)}</td>
                          <td className="px-4 py-3 text-right text-red-600">{formatCurrency(month.churnedARR)}</td>
                        ...
                        <tr className="border-b bg-blue-50">
                          ...
                          <td className="px-4 py-3 text-right text-green-600">{formatCurrency(calculateMetrics.mostLikely.monthly[idx].upsell)}</td>
                          <td className="px-4 py-3 text-right text-red-600">{formatCurrency(calculateMetrics.mostLikely.monthly[idx].churnedARR)}</td>
                        ...
                        <tr className="border-b bg-orange-50">
                          ...
                          <td className="px-4 py-3 text-right text-green-600">{formatCurrency(calculateMetrics.worstCase.monthly[idx].upsell)}</td>
                          <td className="px-4 py-3 text-right text-red-600">{formatCurrency(calculateMetrics.worstCase.monthly[idx].churnedARR)}</td>
